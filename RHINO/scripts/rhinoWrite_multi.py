...
def load_log_metadata(log_root, run_time_prefix):

    log_file_candidate = log_root / run_time_prefix

    if not log_file_candidate.exists():
        print(f"WARNING: no log file found for {run_time_prefix}; continuing without log metadata")
        return {}
    ...
    try:
        ...
        print(f"Loaded log metadata from: {log_file_candidate.name}")
        return parsed_log

    except Exception as e:
        print(f"WARNING: failed to read/parse log file {log_file_candidate.name}: {e}")
        print("Continuing without log metadata for this run")
        return {}
    ...
    try:
        ...
        for tfile in sorted(data_root.glob("*_FuelCycle_T.pkl")):
            try:
                ...
                # log metadata is optional
                parsed_log = load_log_metadata(log_root, run_time_prefix) if log_root.exists() else {}
                ...
                out_name = f"{safe_param}_{run_prefix_data}.bp5"
