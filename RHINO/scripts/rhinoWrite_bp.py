...
# Labels for T and D
labels_T_list = json.dumps([str(x) for x in T_ts_df.index.tolist()])
labels_D_list = json.dumps([str(x) for x in D_ts_df.index.tolist()])
...
OUTPUT_PATH = "/home/ccb/ccb/Projects/IFE_AmSC/RHINO/output/rhino_block.bp5"
...
'''
# series = io.Series(OUTPUT_PATH, io.Access.create, adios2_cfg)
...
series.set_attribute("labels_T_rows", labels_T_list)
series.set_attribute("labels_D_rows", labels_D_list)
if dt is not None:
    series.set_attribute("meta_dt", float(dt))
series.set_attribute("meta_dt_unit", "day")
series.set_attribute("meta_json", json.dumps(meta))
for k, v in meta.items():
    if isinstance(v, (int, float, str, bool, np.integer, np.floating)):
        series.set_attribute(f"meta_{k}", v)
...
for b in range(nblocks):
    ...
    it.set_attribute("block_start_step", np.int64(int(t0)))
    it.set_attribute("block_len", np.int64(int(this_len)))
